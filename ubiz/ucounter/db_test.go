...
func TestDBCounter_Incr(t *testing.T) {
	type args struct {
		ctx    context.Context
		key    CounterNameSpaceType
		megID  uint64
...
func TestDBCounter_Desc(t *testing.T) {
	type args struct {
		ctx    context.Context
		key    CounterNameSpaceType
		megID  uint64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := counter.Decr(tt.args.ctx, tt.args.megID, tt.args.number)
			if err != nil {
				t.Error(err)
			}
			fmt.Println(res)
...
func TestDBCounter_Get(t *testing.T) {
	type args struct {
		ctx   context.Context
		key   CounterNameSpaceType
		megID uint64
